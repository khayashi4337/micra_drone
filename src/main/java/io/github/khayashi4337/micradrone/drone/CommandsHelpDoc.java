...
public final class CommandsHelpDoc {
    ...
    public static final String COMMANDS = """
            ...
            pair_with("north_field")
                このプロットのマーカーが、指定したIDのマーカー(get_plot_id()と
                同じ形式。金床で付けた別名、または自動採番の連番)と
                ペアになりたいと宣言する。片方が宣言しただけでは成立しない
                ―相手側のプロットでも pair_with() でこちら側のIDを
                指定して初めて、双方向のペアとして成立する(見知らぬ相手の
                マーカーに勝手に信号を送れないようにするため)。
                pair_with("") で解除できる。

            is_paired()
                双方向のペアが今まさに成立しているかを調べる。戻り値: true/false。

                ペアが成立していると、set_output()は自分のマーカーだけで
                なく相手のマーカーにも同じ値を伝える(距離を問わないワイヤレス
                redstone)。相手から届いた信号で自分のマーカーが点灯するので、
                それを自分のレバー入力の配線につなげておけば、「相手が合図を
                送ってきたら自分のスクリプトが自動で動き出す」という
                イベント駆動的な使い方もできる(新しい言語機能ではなく、
                既存のレバー入力の仕組みをそのまま使う)。

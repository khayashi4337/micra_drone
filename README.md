...
Corner Markerは置いた瞬間に一意なIDを自動で持つ。金床で名前を付けてから
置くと、その名前で識別できるようになる（他のマーカーと同じ名前は拒否され、
チャットで理由が表示される）。名前を変えたい・消したい場合は、マーカーを
壊して（付けた名前を保持したまま道具箱に戻る）金床で付け直し、再度置けばよい。

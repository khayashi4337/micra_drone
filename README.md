...
授かる。空の巻物を持ってエンチャントテーブルを右クリックすると候補一覧が
開き、ラピスラズリと引き換えに書き込んでもらえる。テーブルの周りに置く
**本棚の数が多いほど、選べる高度なサンプルが増える**（バニラのエンチャント
と同じ仕組み）。テーブル周りのチェスト・シュルカーボックス・彫刻入りの
本棚などに書き込み済みの巻物を入れておくと、それをラピス1個で複製する
こともできる。
...
- **コントローラの隣にレバーを置くと、ON=選択中のスクリプトを実行、
  OFF=停止**できる（GUIを開かずに動かせる）。IDE内にもSave / Run /
  Save & Run / Stopボタンがあり、編集しながらすぐ試せる。

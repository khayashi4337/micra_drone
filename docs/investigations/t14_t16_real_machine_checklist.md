...
## 第3ラウンド(2026-08-28〜29): 実機フィードバックを受けたUI改修と確認

林さんの実機テストで出た指摘を順に反映し、それぞれdevkit経由で確認した:

- [x] **組み込みスクリプト**: 置いたばかりのコントローラでもSave/Runできる
  (`Controller script`が一覧先頭・既定選択)。保存→閉じる→開き直しで復元。
- [x] **レバー×インベントリ巻物**: レバーはオーナーのインベントリから読む
  (林さんのレバー操作で`[run] running inv:7`を確認)。
- [x] **AIへの文脈**: プロット座標対応式、ヘルプ2/3、アンロック/ポイント/未購入品。
- [x] **diffレビュー**: 返答到着時に自動で赤/緑表示、ハンク個別の「x Reject」、
  「Accept rest」「Reject all」、レビュー中は編集ロック。Insertボタンは廃止。
- [x] **AI: thinking...**(オレンジ、ドット巡回)とSend/Compactの非活性、
  **Esc中断**(文面は入力欄に残る、残存プロセスなし)。
- [x] **Danger ON = 提案をそのまま適用+Undo AI change**。
- [x] Region Pointer/Corner Markerの専用テクスチャ、JEIへの登録。
- [ ] **未アンロック作物の本物の種フォールバック**(`plant()`がオーナーの手持ちから
  1個消費): devkitでインベントリに種を入れる手段が無く実機未確認(単体テストのみ)。

## 未確認・残タスク

- 本物の種フォールバック(上記)の実機確認: 未解放の作物(例: pumpkin)の種を持って
  `plant("pumpkin")`を実行し、種が1つ減って茎が植わることを見る

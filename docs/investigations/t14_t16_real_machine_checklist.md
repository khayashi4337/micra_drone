...
- [x] **利用シナリオ(範囲指定→収穫スクリプト生成)**(第3ラウンド):
  `/select-region`でコントローラ周辺13x3x13を選択→Chatを開くと座標が入力欄に
  入る→「この範囲にある小麦を収穫するスクリプトを書いて」→AIが自分で
  `get_block_snapshot`を呼び、「小麦は gx=2..4 の列、gx=1 に穴とシュルカー
  ボックス」と実際の畑の状態を言い当て、プロット情報(ワールド座標⇔グリッド
  座標の対応式、この回で文脈に追加)に基づく蛇行収穫スクリプトを生成→Insertで
  エディタに反映されることを確認。Save & Run→ドローン動作は人手確認に委ねる。
...
- [x] **Region Pointerの選択プレビュー**(Createの粘着剤選択を参考に実装):
  `/select-region`で2x1x2を選び、IDEの上空カメラ越しにスクリーンショット→
  半透明シアンの面+輪郭のボックスと、始点ブロックの強調枠が描画されることを
  確認。始点のみの状態で照準先まで伸びるライブプレビューは、3D照準を合成入力で
  動かせないため人手確認に委ねる。
- [x] **JEIにRegion Pointerとレシピが出る**: クリエイティブタブ登録漏れを修正し、
  JEI検索「region」で表示されることを林さんが目視確認。

## 未確認・残タスク

- Region Pointerのライブプレビュー(始点クリック後、照準先まで伸びる箱)と
  アクションバーの案内文(人手確認)

...
- **An in-game IDE that behaves like one** — syntax highlighting in a Monokai
  ...
  an autocomplete popup as you type a command name, undo/redo (Ctrl+Z / Ctrl+Y),
  run and step buttons on the editor's title bar, rename-in-place by
  double-clicking the script name,
